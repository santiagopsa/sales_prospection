...
//
//   node sdr/cli.js vox:setup [--key sdr/voximplant-key.json] [--url https://peaku-sandler.onrender.com] [--numero +57...]
//
...
async function setup({ key, url, numero, config, log = console.log }) {
  ...
  // Usuario de Angie: la contraseña se genera aquí y solo se imprime; si ya existía, se rota
  // para que siempre coincida con lo que va a Render.
  const password = clave(18);
  const usuarios = (await api('GetUsers', { application_id: appId, user_name: USUARIO })).result;
  if (usuarios.length) { await api('SetUserInfo', { user_id: usuarios[0].user_id, user_password: password }); log(`Usuario "${USUARIO}" ya existía: contraseña rotada`); }
  ...
  try { await api('BindPhoneNumberToApplication', { phone_number_id: tel.phone_id, application_id: appId, bind: 1 }); log(`Número ${callerId} amarrado a la aplicación`); }
