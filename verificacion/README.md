...
- **El porqué del nivel son dos renglones: qué demostró y qué lo separa del siguiente.** "Por qué 4 y no 5" es lo primero que pregunta el cliente, y un párrafo de 55 palabras no lo contestaba. `demostro` (≤30 palabras) y `brecha` (≤25, vacía solo en 5) se imprimen como *✓ Demostró* y *△ Para llegar a N*, con una barra de cinco tramos al lado y otra por requisito en el encabezado. Las actas anteriores siguen mostrando su párrafo: el snapshot no se reescribe.
